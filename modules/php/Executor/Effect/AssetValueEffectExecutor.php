  ...
  public function applyEffect(?int $player_id, Effect $effect): void {
    ...
    if ($effect->condition->evaluate() == ConditionStatus::PASS) {
      ...
      if ($effect instanceof Gain) {
        IndustrialistManager::incPlayerCounter($player_id, IndustrialistManager::COUNTER_INDUSTRIALIST_ASSET_VALUE, $delta);
      }
      else {
        IndustrialistManager::decPlayerCounter($player_id, IndustrialistManager::COUNTER_INDUSTRIALIST_ASSET_VALUE, $delta);
      }
    }
  }
}